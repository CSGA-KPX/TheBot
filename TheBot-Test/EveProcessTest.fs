...
let ``EVE : recipe from bp me:2`` () =
    let ret =
        rm.TryGetRecipeMe(ec.TryGetByName("噩梦级蓝图").Value, ByItem 1.0)
...
let ``EVE : recipe from item me:2`` () =
    let ret =
        rm.TryGetRecipeMe(ec.TryGetByName("噩梦级").Value, ByItem 1.0)
...
let ``EVE : recipe rec from item me:2_Control`` () =
    let ret =
        rm.TryGetRecipeMe(ec.TryGetByName("恶狼级").Value, ByItem 1.0)
...
let ``EVE : recipe rec from item me:2`` () =
    let ret =
        rm.TryGetRecipeRecMe(ec.TryGetByName("恶狼级").Value, ByItem 1.0)

    Assert.IsTrue(ret.IsSome)

    let d =
        process2Dict (ret.Value.FinalProcess.Process)
