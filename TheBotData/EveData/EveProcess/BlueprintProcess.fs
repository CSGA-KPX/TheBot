...
type BlueprintCollection private () = 
    ...
    member private x.TryReadProc(o : JObject) = 
        ...
        if a.ContainsKey("manufacturing") || a.ContainsKey("reaction") then
            ...
            if output.Count = 1
               ...
               Some { Id = bpid
                      Process = {Input = input.ToArray(); Output = output.ToArray()}
                      Type = procType }
            else
                None
        else
    ...
    member x.GetAllProcesses() = x.DbCollection.FindAll() |> Seq.map (fun proc -> proc.AsEveProcess())
