...
type EveProcessManager (cfg : IEveCalculatorConfig) as x = 
    ...
    static let instance = EveProcessManager({new IEveCalculatorConfig with
                                                member x.InputME = 0
    ...
    member private x.ApplyQuantity(recipe : EveProcess, quantity : ProcessQuantity) = 
        if recipe.Flag.HasFlag(ProcessFlags.MeApplied) then
            invalidOp "已经计算过材料效率"
        if recipe.Flag.HasFlag(ProcessFlags.QuantityApplied) then
            invalidOp "已经调整过流程数"

        let runs = quantity.ToRuns(recipe.Process)
        {recipe with
            Process = {Input = recipe.Process.Input
                               |> Array.map (fun mr -> {mr with Quantity = mr.Quantity * runs})
                       Output = recipe.Process.Output
                               |> Array.map (fun mr -> {mr with Quantity = mr.Quantity * runs})}
    ...
    member x.TryGetRecipeMe(item : EveType, quantity, ?ime : int) = 
        ...
        |> Option.map (fun recipe ->
            x.ApplyMe(x.ApplyQuantity(recipe, quantity), ime))
    ...
    member x.TryGetRecipeRecMe(item : EveType, quantity : ProcessQuantity, ?ime : int, ?dme : int) = 
        ...
        |> Option.map (fun r ->
            ...
            let final = { Process = acc.AsRecipeProcess()
                          Type = ProcessType.Invalid
                          Flag = ProcessFlags.MeApplied ||| ProcessFlags.QuantityApplied }
            {| FinalProcess = final
