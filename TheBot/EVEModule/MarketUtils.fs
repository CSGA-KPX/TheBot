...
let GetPriceTable() = 
    AutoTextTable<EveType * float>(
        [|  "名称", fun (t, q) -> t.Name |> box
            "数量", fun (t, q) -> q |> HumanReadableFloat |> NumberCell |> box
            PriceFetchMode.Sell.ToString() + "/税后", fun (t, q) -> 
                ...
                sprintf "%s/%s"  nt st |> NumberCell |> box
            PriceFetchMode.Buy.ToString() + "/税后", fun (t, q) -> 
                ...
                sprintf "%s/%s" nt wt |> NumberCell |> box
            "日均交易", fun (t, q) -> t.GetTradeVolume() |> HumanReadableFloat |> NumberCell |> box
