...
type EatModule() =
    ...
    member x.HandleEat(cmdArg : CommandEventArgs) =
        let at = cmdArg.MessageEvent.Message.GetAts() |> Array.tryHead

        use ret = cmdArg.OpenResponse()
        
        match at with
        | Some Sections.AtUserType.All -> ret.AbortExecution(InputError, "你要请客吗？")
        | Some (Sections.AtUserType.User uid) when uid = cmdArg.SelfId || uid = cmdArg.MessageEvent.UserId ->
        ...
        | Some (Sections.AtUserType.User uid) ->
            let atUserName = GetGroupMemberInfo(cmdArg.MessageEvent.GroupId, uid)
        ...
        | 1 -> 
            ...
            if eatFuncs.ContainsKey(str) then
                ...
                for l in whenToEat(dicer, cmdArg.Arguments) do ret.WriteLine(l)
        | _ ->
            for l in whenToEat(dicer, cmdArg.Arguments) do ret.WriteLine(l)
