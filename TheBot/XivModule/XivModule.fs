...
type XivModule() =
        ...
        if uo.IsDefined("rebuild") then
           ...
           cmdArg.QuickMessageReply(sprintf "重建完成，当前有%i个副本" dfcRoulettes.Length)
            ...
            if dfcRoulettes.Length = 0 then
                    cmdArg.AbortExecution(ModuleError, "模块错误：副本表为空。请使用rebuild")
    ...
    member x.HandleFantasia(cmdArg : CommandEventArgs) = 
        let choices = 
            [|
                ...
                "大猫"; "兔子"
            |]
        let atUser = cmdArg.MessageEvent.Message.GetAts() |> Array.tryHead
        if atUser.IsSome then
            match atUser.Value with
            | Sections.AtUserType.All ->
                cmdArg.AbortExecution(InputError, "全员发洗澡水？给我一瓶谢谢！")
            | Sections.AtUserType.User i when i = cmdArg.SelfId ->
                cmdArg.AbortExecution(InputError, "请联系开发者")
            | Sections.AtUserType.User _ ->
                cmdArg.AbortExecution(ModuleError, "暂不支持")

        let dicer = new Dicer(SeedOption.SeedByUserDay(cmdArg.MessageEvent))
    ...
    member x.HandleCGSS(cmdArg : CommandEventArgs) =
        ...
        if job.IsNone || iLv.IsNone then
            cmdArg.AbortExecution(InputError, "请提供职业和品级信息。职业可以使用：单字简称/全程/英文简称")
    ...
    member x.HandleNrnr(cmdArg : CommandEventArgs) = 
        ...
        using (cmdArg.OpenResponse(cfg.IsImageOutput)) (fun ret -> 
            ret.Write(title)
            ret.Write("\r\n")
