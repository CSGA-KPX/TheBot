...
type DiceModule() =
    ...
    member x.HandleChoices(cmdArg : CommandEventArgs) =
        ...
        if atUser.IsSome then
            ...
            | Sections.AtUserType.User x when x = cmdArg.SelfId
                                          && not <| cmdArg.RawMessage.Contains(loginInfo.Nickname) ->
                sw.WriteLine("公投：")
            | Sections.AtUserType.User x ->
                let atUserName = GetGroupMemberInfo(cmdArg.MessageEvent.GroupId, x)
        ...
        |> Array.map (fun c -> 
            let seed = 
                if cmdArg.CommandName = "cc" then Array.singleton SeedOption.SeedRandom 
                    ...
                    if atUser.IsSome then SeedOption.SeedByAtUserDay(cmdArg.MessageEvent)
    ...
    member x.HandleChoice(cmdArg : CommandEventArgs) = 
        ...
        for arg in cmdArg.Arguments do 
            ...
            if succ then
                let i = int(i)
                if i > 100 then cmdArg.AbortExecution(InputError, "最多100道")
                else count <- i
        let choices = [|"A"; "B"; "C"; "D"|]
        let chunks = 
