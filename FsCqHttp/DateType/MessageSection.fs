...
type MessageSection(typeName : string) = 
    ...
    member internal x.ParseFrom(sec : JObject) = 
        ...
        if (x.TypeName <> "") && (x.TypeName <> typeName) then
            invalidArg "type" (sprintf "type字段不匹配：需求%s，实际%s" x.TypeName typeName)
    ...
    static member internal CreateFrom(sec : JObject) = 
        ...
        if not <| sectionInfoCache.ContainsKey(typeName) then 
            typeName <- ""
    ...
    static member internal CreateFrom(typeName : string, values : seq<string * string>) =
        ...
        let t = sectionInfoCache.[typeName]
        let obj = Activator.CreateInstance(t) :?> MessageSection
        for (name, value) in values do obj.SetValue(name, value)
        obj

/// 用于储存类型未知消息段
type RawMessageSection() = inherit MessageSection("")
...
/// 掷骰子魔法表情
type DiceSection() = inherit MessageSection("dice")

/// 猜拳魔法表情
type RpsSection() = inherit MessageSection("rps")

/// 窗口抖动
type ShakeSection() = inherit MessageSection("rps")
