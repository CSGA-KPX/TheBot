...
type private DailySanCacheCollection private () =
    let cache = ConcurrentDictionary<uint64, DailySanCacheItem>()
    ...
    member x.GetValue(cmdArg: CommandEventArgs) =
        let seed = SeedOption.SeedByUserDay(cmdArg.MessageEvent)
        ...
        if not succ || (item.Key <> seedString) then
            ...
            cache.AddOrUpdate(uid.Value, newItem, (fun _ _ -> newItem)) |> ignore
...
type DailySanModule() =
    ...
    member x.HandleSanCheck(cmdArg: CommandEventArgs) =
        ...
        if args.Length <> 1 || (not <| args.[0].Contains("/")) then
